...
echo ""
echo "نصب بسته شبکه ملی سلامت (Hyperledger Fabric)"
echo "  مقصد: $TARGET"
...
if [ ! -d "$TARGET" ]; then
  ...
  echo "    git clone https://github.com/mohammadlohrasbi/health-network.git $TARGET"
  exit 1
fi
for d in scripts server public config; do
  if [ ! -d "$TARGET/$d" ]; then
    bad "$TARGET/$d نیست — این مخزن health-network نیست."
    exit 1
  fi
done

...
# /root/health-network/scripts/network.sh
...
setup_network_with_fabric_ca_tls_nodeous_active() {
  ...
  # ایجاد شبکه Docker
  # =====================================================
  if ! docker network ls | grep -q "health-network"; then
    log "ایجاد شبکه health-network"
    docker network create health-network
    success "شبکه health-network ساخته شد"
  else
    log "شبکه health-network از قبل وجود دارد"
  fi
...
EOF
  ...
  _enroll_tls() {
    ...
    mkdir -p "$out"
    docker run --rm --network health-network \
      -v "$CRYPTO_DIR":/crypto-config \
      hyperledger/fabric-ca-tools:latest \
      /bin/bash -c "
        set -e
